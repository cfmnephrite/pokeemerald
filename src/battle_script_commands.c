...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if ((gStatuses3[gBattlerTarget] & STATUS3_PHANTOM_FORCE)
        ...
        || (!(move == MOVE_EARTHQUAKE || move == MOVE_MAGNITUDE || move == MOVE_FISSURE) && gStatuses3[gBattlerTarget] & STATUS3_UNDERGROUND))
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        JumpIfMoveFailed(7, move);
        return TRUE;
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_ABILITIES: // Such as abilities activating on contact(Poison Spore, Rough Skin, etc.).
            ...
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_ABILITIES_ATTACKER: // Gooey, Poison Touch, Hyper Cutter
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_ACUPRESSURE:
        ...
        if (bits)
        {
            ...
            do
            {
                statId = (Random() % NUM_STATS) + 1;
            } while (!(bits & gBitTable[statId]));

            SET_STATCHANGER(statId, (gBattleMons[gActiveBattler].statStages[statId] >= 11 ? 1 : 2), FALSE);
            gBattlescriptCurrInstr += 7;
        }
        else
        {
