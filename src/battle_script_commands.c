...
static void Cmd_setvolatilestatus(void);
...
static const u32 sStatusFlagsForMoveEffects[NUM_MOVE_EFFECTS] =
{
    ...
    [MOVE_EFFECT_CONFUSION]      = VOLATILE_STATUS_CONFUSION,
    [MOVE_EFFECT_FLINCH]         = VOLATILE_STATUS_FLINCHED,
    [MOVE_EFFECT_UPROAR]         = VOLATILE_STATUS_UPROAR,
    [MOVE_EFFECT_CHARGING]       = VOLATILE_STATUS_MULTIPLETURNS,
    [MOVE_EFFECT_WRAP]           = VOLATILE_STATUS_WRAPPED,
    [MOVE_EFFECT_RECHARGE]       = VOLATILE_STATUS_RECHARGE,
    [MOVE_EFFECT_PREVENT_ESCAPE] = VOLATILE_STATUS_ESCAPE_PREVENTION,
    [MOVE_EFFECT_NIGHTMARE]      = VOLATILE_STATUS_NIGHTMARE,
    [MOVE_EFFECT_THRASH]         = VOLATILE_STATUS_LOCK_CONFUSE,
};
...
static void Cmd_attackcanceler(void)
{
    ...
    if (!gBattleMons[gBattlerAttacker].pp[gCurrMovePos] && gCurrentMove != MOVE_STRUGGLE
     ...
     && !(gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns))
    {
    ...
    if ((NoTargetPresent(gBattlerAttacker, gCurrentMove)
        && (!gBattleMoveEffects[effect].twoTurnEffect || (gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns)))
        || (IsMoveNotAllowedInSkyBattles(gCurrentMove)))
    {
        ...
        if (!gBattleMoveEffects[effect].twoTurnEffect || (gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns))
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if (gBattleMons[battlerDef].volatileStatuses.foresight || gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED)
        buff = accStage;
    else
...
static void Cmd_ppreduce(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns)
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, enum ItemHoldEffect holdEffectAtk)
{
    ...
    {
        critChance  = 2 * ((gBattleMons[battlerAtk].volatileStatuses.focusEnergy) != 0)
                    + 1 * ((gBattleMons[battlerAtk].volatileStatuses.dragonCheer) != 0)
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, enum ItemHoldEffect holdEffectAtk)
{
    ...
    if (gBattleMons[battlerAtk].volatileStatuses.focusEnergy)
        critChance *= 4;
    else if (gBattleMons[battlerAtk].volatileStatuses.dragonCheer)
        critChance *= 2;
...
static void Cmd_attackanimation(void)
{
    ...
    else
    {
        ...
        {
            u32 multihit;
            if (gBattleMons[gBattlerTarget].volatileStatuses.substitute)
            {
...
void SetMoveEffect(bool32 primary, bool32 certain)
{
    ...
    {
        if (GetMonVolatileStatus(gEffectBattler, sStatusFlagsForMoveEffects[gBattleScripting.moveEffect]))
        {
            gBattlescriptCurrInstr++;
        }
        else
        {
            ...
            case MOVE_EFFECT_CONFUSION:
                ...
                {
                    gBattleMons[gEffectBattler].volatileStatuses.confusionTurns = ((Random()) % 4) + 2; // 2-5 turns
                    ...
                    {
                        gBattleMons[gEffectBattler].volatileStatuses.lockConfusionTurns = 0;
            ...
            case MOVE_EFFECT_UPROAR:
                if (!(gBattleMons[gEffectBattler].volatileStatuses.uproarTurns))
                {
                    gBattleMons[gEffectBattler].volatileStatuses.multipleTurns = TRUE;
                    gLockedMoves[gEffectBattler] = gCurrentMove;
                    gBattleMons[gEffectBattler].volatileStatuses.uproarTurns = B_UPROAR_TURNS >= GEN_5 ? 3 : ((Random() & 3) + 2);
            ...
            case MOVE_EFFECT_WRAP:
                if (gBattleMons[gEffectBattler].volatileStatuses.wrapped)
                {
                    gBattlescriptCurrInstr++;
                }
                else
                {
                    gBattleMons[gEffectBattler].volatileStatuses.wrapped = TRUE;
            ...
            case MOVE_EFFECT_RECHARGE:
                ...
                gBattleMons[gEffectBattler].volatileStatuses.recharge = TRUE;
                gDisableStructs[gEffectBattler].rechargeTimer = 2;
                gLockedMoves[gEffectBattler] = gCurrentMove;
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_RAGE:
                gBattleMons[gBattlerAttacker].volatileStatuses.rage = TRUE;
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_STEAL_ITEM:
            ...
            case MOVE_EFFECT_PREVENT_ESCAPE:
                gBattleMons[gBattlerTarget].volatileStatuses.escapePrevention = TRUE;
                gDisableStructs[gBattlerTarget].battlerPreventingEscape = gBattlerAttacker;
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_NIGHTMARE:
                gBattleMons[gBattlerTarget].volatileStatuses.nightmare = TRUE;
                gBattlescriptCurrInstr++;
                break;
            ...
            case MOVE_EFFECT_THRASH:
                ...
                else
                {
                    gBattleMons[gEffectBattler].volatileStatuses.multipleTurns = TRUE;
                    gLockedMoves[gEffectBattler] = gCurrentMove;
                    gBattleMons[gEffectBattler].volatileStatuses.lockConfusionTurns = RandomUniform(RNG_RAMPAGE_TURNS, 2, 3);
                }
                break;
            ...
            case MOVE_EFFECT_TRAP_BOTH:
                if (!gBattleMons[gBattlerTarget].volatileStatuses.escapePrevention && !gBattleMons[gBattlerAttacker].volatileStatuses.escapePrevention)
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_BothCanNoLongerEscape;
                }
                if (!gBattleMons[gBattlerTarget].volatileStatuses.escapePrevention)
                    gDisableStructs[gBattlerTarget].battlerPreventingEscape = gBattlerAttacker;

                if (!gBattleMons[gBattlerAttacker].volatileStatuses.escapePrevention)
                    gDisableStructs[gBattlerAttacker].battlerPreventingEscape = gBattlerTarget;

                gBattleMons[gBattlerTarget].volatileStatuses.escapePrevention = TRUE;
                gBattleMons[gBattlerAttacker].volatileStatuses.escapePrevention = TRUE;
                break;
            ...
            case MOVE_EFFECT_LEECH_SEED:
                if (!IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) && !gBattleMons[gBattlerTarget].volatileStatuses.leechSeed)
                {
                    gBattleMons[gBattlerTarget].volatileStatuses.leechSeed = gBattlerAttacker + 1;
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                ...
                if (gLastMoves[gBattlerTarget] != MOVE_NONE && gLastMoves[gBattlerTarget] != 0xFFFF)
                {
                    ...
                    if (i != MAX_MON_MOVES && gBattleMons[gBattlerTarget].pp[i] != 0)
                    {
                        ...
                        if (!(gDisableStructs[gBattlerTarget].mimickedMoves & (1u << i))
                            && !(gBattleMons[gBattlerTarget].volatileStatuses.transformed))
                        {
            ...
            case MOVE_EFFECT_FIRE_SPIN_SIDE:
            {
                ...
                for (battler = 0; battler < MAX_BATTLERS_COUNT; ++battler)
                {
                    if (!IsBattlerAlly(battler, gBattlerTarget))
                        continue;
                    if (!gBattleMons[battler].volatileStatuses.wrapped)
                    {
                        gBattleMons[battler].volatileStatuses.wrapped = TRUE;
                        if (GetBattlerHoldEffect(gBattlerAttacker, TRUE) == HOLD_EFFECT_GRIP_CLAW)
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_RAGE: // rage check
            if (gBattleMons[gBattlerTarget].volatileStatuses.rage
                && IsBattlerAlive(gBattlerTarget)
                && gBattlerAttacker != gBattlerTarget
                && !IsBattlerAlly(gBattlerAttacker, gBattlerTarget)
                && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
        ...
        case MOVEEND_SUBSTITUTE:
            for (i = 0; i < gBattlersCount; i++)
            {
                if (gDisableStructs[i].substituteHP == 0)
                    gBattleMons[i].volatileStatuses.substitute = FALSE;
            }
            gBattleScripting.moveendState++;
            break;
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            if ((gBattleStruct->moveResultFlags[gBattlerTarget] & (MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE))
             || (gBattleMons[gBattlerAttacker].volatileStatuses.flinched)
        ...
        case MOVEEND_CLEAR_BITS: // Clear/Set bits for things like using a move for all targets and all hits.
            ...
            if (B_RAMPAGE_CANCELLING >= GEN_5
              ...
              && gBattleMons[gBattlerAttacker].volatileStatuses.lockConfusionTurns != 1)  // And won't end this turn
...
{
    gBattleMons[battler].volatileStatuses.destinyBond = FALSE;
    gHitMarker &= ~HITMARKER_DESTINYBOND;
    gBattleScripting.battler = battler;
...
static void Cmd_handlelearnnewmove(void)
{
    ...
    else
    {
        ...
        if (gBattlerPartyIndexes[battler] == monId
            && !(gBattleMons[battler].volatileStatuses.transformed))
        {
...
static bool32 TryTidyUpClear(u32 battlerAtk, bool32 clear)
{
    ...
    {
        if (gBattleMons[i].volatileStatuses.substitute)
        {
            if (clear)
            {
                gBattlerTarget = i;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_INFATUATE_WITH_BATTLER:
    {
        ...
        gBattleScripting.battler = battler;
        gBattleMons[battler].volatileStatuses.infatuation = GetBattlerForBattleScript(cmd->infatuateWith) + 1;
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_SET_SKY_DROP:
    {
        ...
        // End any multiturn effects caused by the target except VOLATILE_STATUS_LOCK_CONFUSE
        gBattleMons[gBattlerTarget].volatileStatuses.multipleTurns = 0;
        gBattleMons[gBattlerTarget].volatileStatuses.uproarTurns= 0;
        gBattleMons[gBattlerTarget].volatileStatuses.bideTurns = 0;
    ...
    case VARIOUS_CLEAR_SKY_DROP:
    {
        ...
        if (gBattleMons[gBattlerTarget].volatileStatuses.lockConfusionTurns != 0)
    ...
    case VARIOUS_SKY_DROP_YAWN: // If the mon that's sleeping due to Yawn was holding a Pokemon in Sky Drop, release the target and clear Sky Drop data.
    {
        ...
        if (gBattleStruct->skyDropTargets[gEffectBattler] != SKY_DROP_NO_TARGET && !(gStatuses3[gEffectBattler] & STATUS3_SKY_DROPPED))
        {
            ...
            if (gBattleMons[gEffectBattler].volatileStatuses.lockConfusionTurns != 0 && CanBeConfused(gEffectBattler))
            {
                gBattleMons[gEffectBattler].volatileStatuses.lockConfusionTurns = 0;
                gBattlerAttacker = gEffectBattler;
                gBattleMons[gBattlerTarget].volatileStatuses.confusionTurns = ((Random()) % 4) + 2;
                gBattlescriptCurrInstr = BattleScript_ThrashConfuses;
                return;
            }
        }
    ...
    case VARIOUS_TRY_NO_RETREAT:
    {
        ...
        {
            if (!gBattleMons[battler].volatileStatuses.escapePrevention)
    ...
    case VARIOUS_CURE_CERTAIN_STATUSES:
    {
        VARIOUS_ARGS();
        // Check infatuation
        if (gBattleMons[battler].volatileStatuses.infatuation)
        {
        ...
        // Check torment
        if (gBattleMons[battler].volatileStatuses.torment == TRUE)
        {
            gBattleMons[battler].volatileStatuses.torment = FALSE;
...
static void Cmd_setseeded(void)
{
    ...
    if (gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT || gBattleMons[gBattlerTarget].volatileStatuses.leechSeed)
    {
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
...
bool8 UproarWakeUpCheck(u8 battler)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (!(gBattleMons[i].volatileStatuses.uproarTurns) || hasSoundproof)
            continue;
...
static void Cmd_setbide(void)
{
    ...
    gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns = TRUE;
    gLockedMoves[gBattlerAttacker] = gCurrentMove;
    gBideDmg[gBattlerAttacker] = 0;
    gBattleMons[gBattlerAttacker].volatileStatuses.bideTurns = 2;
...
static void Cmd_tryinfatuating(void)
{
    ...
    {
        if (gBattleMons[gBattlerTarget].volatileStatuses.infatuation
        ...
        {
            gBattleMons[gBattlerTarget].volatileStatuses.infatuation = gBattlerAttacker + 1;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
    }
}
...
static void Cmd_setfocusenergy(void)
{
    ...
    if ((effect == EFFECT_DRAGON_CHEER && (!(IsDoubleBattle()) || (gAbsentBattlerFlags & (1u << battler))))
         || gBattleMons[battler].volatileStatuses.dragonCheer || gBattleMons[battler].volatileStatuses.focusEnergy)
    {
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_FAILED;
    ...
    {
        gBattleMons[battler].volatileStatuses.dragonCheer = TRUE;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_GETTING_PUMPED;
    }
    else
    {
        if (GetGenConfig(GEN_CONFIG_FOCUS_ENERGY_CRIT_RATIO) >= GEN_3)
            gBattleMons[battler].volatileStatuses.focusEnergy = TRUE;
        else
            gBattleMons[battler].volatileStatuses.dragonCheer = TRUE;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_GETTING_PUMPED;
    }
...
static void Cmd_transformdataexecution(void)
{
    ...
    gBattlescriptCurrInstr = cmd->nextInstr;
    if (gBattleMons[gBattlerTarget].volatileStatuses.transformed
    ...
    else
    {
        ...
        gBattleMons[gBattlerAttacker].volatileStatuses.transformed = TRUE;
...
static void Cmd_mimicattackcopy(void)
{
    ...
    if ((IsMoveMimicBanned(gLastMoves[gBattlerTarget]))
        || (gBattleMons[gBattlerAttacker].volatileStatuses.transformed)
    ...
    if (B_UPDATED_CONVERSION_2 < GEN_5)
    {
         ...
         || gLastLandedMoves[gBattlerAttacker] == MOVE_UNAVAILABLE)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
              ...
              && gBattleMons[gLastHitBy[gBattlerAttacker]].volatileStatuses.multipleTurns)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
...
static void Cmd_copymovepermanently(void)
{
    ...
    if (!(gBattleMons[gBattlerAttacker].volatileStatuses.transformed)
...
static void Cmd_trysetdestinybond(void)
{
    ...
    {
        gBattleMons[gBattlerAttacker].volatileStatuses.destinyBond = TRUE;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static void TrySetDestinyBondToHappen(void)
{
    if (gBattleMons[gBattlerTarget].volatileStatuses.destinyBond
        && !IsBattlerAlly(gBattlerAttacker, gBattlerTarget)
        && !(gHitMarker & HITMARKER_GRUDGE))
    {
        gHitMarker |= HITMARKER_DESTINYBOND;
...
static void Cmd_healpartystatus(void)
{
     ...
     || !(isSoundMove && GetBattlerAbility(gBattlerAttacker) == ABILITY_SOUNDPROOF))
    {
        ...
        gBattleMons[gBattlerAttacker].volatileStatuses.nightmare = FALSE;
    }
    else
    {
        RecordAbilityBattle(gBattlerAttacker, gBattleMons[gBattlerAttacker].ability);
    ...
    if (IsBattlerAlive(partner))
    {
         ...
         || !(isSoundMove && GetBattlerAbility(partner) == ABILITY_SOUNDPROOF))
        {
            ...
            gBattleMons[partner].volatileStatuses.nightmare = FALSE;
        }
        else
        {
...
static void Cmd_cursetarget(void)
{
    ...
    if (gBattleMons[gBattlerTarget].volatileStatuses.cursed)
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        gBattleMons[gBattlerTarget].volatileStatuses.cursed = TRUE;
        gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / 2;
        if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
            gBattleStruct->moveDamage[gBattlerAttacker] = 1;
...
static void Cmd_setvolatilestatus(void)
{
    CMD_ARGS(u8 volatileStatus, u8 battler, u8 value);

    SetMonVolatileStatus(GetBattlerForBattleScript(cmd->battler), cmd->volatileStatus, cmd->value);
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_handlerollout(void)
{
    ...
    {
        if (!(gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns)) // First hit.
        {
        ...
        {
            gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns = FALSE;
        }
...
static void Cmd_jumpifconfusedandstatmaxed(void)
{
    ...
    if (gBattleMons[gBattlerTarget].volatileStatuses.confusionTurns > 0
...
static void Cmd_rapidspinfree(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].volatileStatuses.wrapped)
    {
    ...
    else if (gBattleMons[gBattlerAttacker].volatileStatuses.leechSeed)
    {
        gBattleMons[gBattlerAttacker].volatileStatuses.leechSeed = 0;
...
static void Cmd_settorment(void)
{
    ...
    if (gBattleMons[gBattlerTarget].volatileStatuses.torment == TRUE
        || (GetActiveGimmick(gBattlerTarget) == GIMMICK_DYNAMAX))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    ...
    {
        gBattleMons[gBattlerTarget].volatileStatuses.torment = TRUE;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
void BS_TrySetOctolock(void)
{
    ...
    else
    {
        ...
        gBattleMons[battler].volatileStatuses.escapePrevention = TRUE;
...
void BS_TryRelicSong(void)
{
    ...
    if (GetBattlerAbility(gBattlerAttacker) != ABILITY_SHEER_FORCE && !(gBattleMons[gBattlerAttacker].volatileStatuses.transformed)
...
void BS_CheckPokeFlute(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (GetBattlerAbility(i) != ABILITY_SOUNDPROOF)
        {
            ...
            gBattleMons[i].volatileStatuses.nightmare = FALSE;
        }
    }
