    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_EVS_PLUS_2:
                gBattlescriptCurrInstr++;
                if (gBattleScripting.moveEffect >= MOVE_EFFECT_ATK_PLUS_2)
                    statValue = STAT_BUFF_ATK_2 << (2 * (gBattleScripting.moveEffect - MOVE_EFFECT_ATK_PLUS_2));
                else
                    statValue = STAT_BUFF_ATK_1 << (2 * (gBattleScripting.moveEffect - MOVE_EFFECT_ATK_PLUS_1));
                if (!(NoAliveMonsForEitherParty() || SetStatChangerAndChangeStatBuffs(statValue, affectsUser | STAT_CHANGE_UPDATE_MOVE_EFFECT, 0) == STAT_CHANGE_DIDNT_WORK))
                {
            ...
            case MOVE_EFFECT_ATK_DEF_DOWN: // SuperPower
                if (!NoAliveMonsForEitherParty())
                {
                    SetStatChangerAndChangeStatBuffs((STAT_BUFF_ATK_DEF_1 | STAT_BUFF_NEGATIVE), MOVE_EFFECT_AFFECTS_USER, 0);
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_StatChangeMsg;
                }
                break;
                ...
                if (!NoAliveMonsForEitherParty())
                {
                    ...
                    for (i = STAT_ATK; i < NUM_BATTLE_STATS; i++)
                    {
                        if (gBattleMons[gBattlerTarget].statStages[i] > DEFAULT_STAT_STAGE && gBattleMons[gBattlerAttacker].statStages[i] != MAX_STAT_STAGE)
                        {
                            ...
                            if (boost > 3)
                            {
                                // A little cheat...if boosting goes over 3, add bytes to gBattleStruct->storedStatBuffs
                                gBattleStruct->storedStatBuffs[0] = TRUE;
                                gBattleStruct->storedStatBuffs[i] = boost;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_TRY_DEFIANT_COMPETITIVE:
    {
        ...
        DebugPrintf("Can we try Competitive? %d", gBattleScripting.statChanger);
        ...
        gBattleScripting.stickyWebStatDrop = 0; // this has to be cleared if not activated here
        break;
    }
    ...
    case VARIOUS_TOTEM_BOOST:
    {
        ...
        else
        {
            for (i = 0; i < (NUM_BATTLE_STATS - 1); i++)
            {
                ...
                {
                    // These can be large (thanks to Mirror Herb) so use the storedStatBuffs array
                    gBattleStruct->storedStatBuffs[0] = TRUE;
                    gBattleStruct->storedStatBuffs[i + 1] = gTotemBoosts[gActiveBattler].statChanges[i];
...
{
    u8 result = gBattleStruct->storedStatBuffs[0] ?
...
static u8 GetStatFromBuffStringIndex(u8 index)
{
    switch (index)
    {
        case 2: // Spa
...
{
    u8 i, drop, statsToChange = 0, statsChanged = 0;
    bool8 specificStatLossPrevented = FALSE;

    for (i = STAT_ATK; i < NUM_BATTLE_STATS; i++)
    {
        if ((drop = abs(GetStatBuff(statValue, i, statBuffsHelper->doubled))))
        {
            // Track how many stats we've attempted to change
            // Useful for Hyper Cutter etc. redirects
            statsToChange++;

            // Try various ways to fail
                    ...
                    && statBuffsHelper->notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(0))
            {
                ...
                return 0;
                    ...
                    || statBuffsHelper->activeBattlerAbility == ABILITY_FULL_METAL_BODY
                    || statBuffsHelper->activeBattlerAbility == ABILITY_WHITE_SMOKE)
                    && (!statBuffsHelper->affectsUser || statBuffsHelper->mirrorArmored)
                    && !statBuffsHelper->certain && gCurrentMove != MOVE_CURSE)
            {
            ...
            else if ((statBuffsHelper->index = IsFlowerVeilProtected(gActiveBattler)) && !statBuffsHelper->certain)
            {
                ...
                return 0;
                    ...
                    || (statBuffsHelper->activeBattlerAbility == ABILITY_HYPER_CUTTER && i == STAT_ATK)
                    || (statBuffsHelper->activeBattlerAbility == ABILITY_BIG_PECKS && i == STAT_DEF)))
            {
                if (flags == STAT_CHANGE_ALLOW_PTR)
                {
                    // If we have other stats to change then we want to come back here
                    // For example, Hyper Cutter might stop Tickle from lowering Attack
                    // but we still want to try lowering defense!
                    PREPARE_STAT_BUFFER(gBattleTextBuff2, i);
                    gBattlerAbility = gActiveBattler;
                    specificStatLossPrevented = TRUE;
                }
            }
            else if (statBuffsHelper->activeBattlerAbility == ABILITY_MIRROR_ARMOR && !statBuffsHelper->affectsUser
                && !statBuffsHelper->mirrorArmored && gBattlerAttacker != gBattlerTarget && gActiveBattler == gBattlerTarget)
            {
                if (flags == STAT_CHANGE_ALLOW_PTR)
                {
                    gBattleScripting.battler = gBattlerAttacker;
            ...
            {
                if ((drop = min(gBattleMons[gActiveBattler].statStages[i], drop)))
                {
                    ...
                    switch (drop)
                    {
                        case 1:
                            if (!(*statAnimId))
                                *statAnimId = STAT_ANIM_MINUS1 + i - 1;
                            else if ((*statAnimId) && (*statAnimId) < STAT_ANIM_MINUS2)
                                *statAnimId = STAT_ANIM_MULTIPLE_MINUS1;
                            statBuffsHelper->statBuffStrings[GetStringIndexForStatBuff(i)] = STAT_CHANGE_FELL;
                            break;
                        default:
                            if (!(*statAnimId))
                                *statAnimId = STAT_ANIM_MINUS2 + i - 1;
                            else
                                *statAnimId = STAT_ANIM_MULTIPLE_MINUS2;                    // or STAT_CHANGE_SEVERELY_FELL
                ...
                {
                    statBuffsHelper->statBuffStrings[GetStringIndexForStatBuff(i)] = STAT_CHANGE_WONT_GO_LOWER;
                }
            }
        }
    }

    // Handle redirect for Hyper Cutter etc.
    if (specificStatLossPrevented)
    {
        if (statsToChange > 1)
            BattleScriptPush(gBattlescriptCurrInstr);
        else
...
static u8 TryRaiseStats(u16 *statValue, u32 flags, struct StatBuffsHelper *statBuffsHelper, u8 *statAnimId)
{
    ...
    for (i = STAT_ATK; i < NUM_BATTLE_STATS; i++)
    {
        if ((boost = abs(GetStatBuff(statValue, i, statBuffsHelper->doubled))))
        {
    ...
    for (i = 0; i < NUM_BATTLE_STATS - 1; i++)
    {
            ...
            && !(successfulStatBuffCount > 0 && !STAT_CHANGE_SUCCESS(*(statBuffStrings + i))))
        {
            ...
            if (B_FANCY_STAT_STRINGS && !(activeBattlerAbility == ABILITY_DEFIANT || activeBattlerAbility == ABILITY_COMPETITIVE))
            {
                ...
                {
                    if (*(statBuffStrings + j) == *(statBuffStrings + i))
                    {
                        statCountForCurrentString++;
                        switch (j + 1)
                        {
                            case STAT_ACC:
                                if ((*(statBuffStrings + j) == STAT_CHANGE_DRASTICALLY_ROSE && statCountForCurrentString >= 4)
                                    || statCountForCurrentString > 5)
                                    statCountForCurrentString--;
                                break;
                            case STAT_EVASION:
                                if (statCountForCurrentString > 4)
                                {
                                    statCountForCurrentString--;
                                    // If accuracy and evasion are being boosted by the same amount
                                    // on a long string, move them to the next string
                                    // The < 5 check is important - any larger and accuracy will
                                    // have removed itself from the string already (see above)
                                    if ((statCountForCurrentString < 5) && *(statBuffStrings + j) == *(statBuffStrings + j - 1))
                                        statCountForCurrentString--;
                                }
                                break;
                        }
...
static u32 ChangeStatBuffs(u16 *statValue, u32 flags, const u8 *failPtr, u8 *textBuffer)
{
    ...
    if ((*(statValue) & 0x3FFF) || gBattleStruct->storedStatBuffs[0] == TRUE) // make sure that there are actual stats to buff
    {
        ...
        statBuffsHelper.activeBattlerAbility = GetBattlerAbility(gActiveBattler);
    ...
    memset(gBattleStruct->storedStatBuffs, 0, sizeof(gBattleStruct->storedStatBuffs));  // erase this, just to be safe
...
static void Cmd_statbuffchange(void)
{
    CMD_ARGS(u16 flags, const u8 *failPtr);

    // Set initial parameters
    const u8 *nextInstr = cmd->nextInstr;
    const u8 *failPtr;
    gBattlescriptCurrInstr = gBattlescriptCurrInstr = cmd->nextInstr;
    if (cmd->failPtr)
        failPtr = cmd->failPtr;
    else
        failPtr = nextInstr;
    ...
    {
        // We haven't been redirected elsewhere - go to the failPtr
        if (gBattlescriptCurrInstr == nextInstr)
            gBattlescriptCurrInstr = failPtr;

        // We have or we want to skip failure strings anyway
        if (gBattlescriptCurrInstr != nextInstr || (cmd->flags & STAT_CHANGE_SKIP_FAILED_STRINGS))
...
static void Cmd_maxattackhalvehp(void)
{
    ...
    {
        gBattleScripting.statChanger = 0; // wipe this to remove the chance of a negative stat buff
