...
static void Cmd_volatilestatusanimation(void);
...
void (*const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_volatilestatusanimation,                 //0x65
...
static void Cmd_volatilestatusanimation(void)
{
    CMD_ARGS(u8 battler, u8 volatileStatus);
    ...
    if (gBattleControllerExecFlags == 0)
    {
        ...
        if (!(gStatuses3[battler] & STATUS3_SEMI_INVULNERABLE)
            && gDisableStructs[battler].substituteHP == 0
            && GetMonVolatileStatus(battler, cmd->volatileStatus)
            && !(gHitMarker & (HITMARKER_NO_ANIMATIONS | HITMARKER_DISABLE_ANIMATION)))
        {
            BtlController_EmitStatusAnimation(battler, B_COMM_TO_CONTROLLER, TRUE, cmd->volatileStatus);
            MarkBattlerForControllerExec(battler);
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void Cmd_various(void)
{
    ...
    {
        VARIOUS_ARGS();
        gBattleMons[battler].volatileStatuses.powder = TRUE;
        break;
    }
...
static void Cmd_setforesight(void)
{
    ...
    gBattleMons[gBattlerTarget].volatileStatuses.foresight = TRUE;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_setdefensecurlbit(void)
{
    ...
    gBattleMons[gBattlerAttacker].volatileStatuses.defenseCurl = TRUE;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
