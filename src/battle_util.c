...
u32 IsAbilityPreventingEscape(u32 battlerId)
{
    ...
    if ((B_GHOSTS_ESCAPE >= GEN_6 && IS_BATTLER_OF_TYPE(battlerId, TYPE_GHOST)) || gBattleMons[battlerId].ability == ABILITY_RUN_AWAY)
        return 0;
...
bool32 CanBattlerEscape(u32 battlerId) // no ability check
{
    return (GetBattlerHoldEffect(battlerId, TRUE) == HOLD_EFFECT_SHED_SHELL
            || gBattleMons[battlerId].ability == ABILITY_RUN_AWAY
