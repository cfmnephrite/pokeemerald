...
void HandleAction_UseMove(void)
{
    ...
    else if (gBattleMons[gBattlerAttacker].volatileStatuses.multipleTurns || gBattleMons[gBattlerAttacker].volatileStatuses.recharge)
    {
...
const u8 *CheckSkyDropState(u32 battler, enum SkyDropState skyDropState)
{
    ...
    if (gBattleMons[otherSkyDropper].volatileStatuses.lockConfusionTurns != 0 && skyDropState != SKY_DROP_STATUS_YAWN)
    {
        gBattleMons[otherSkyDropper].volatileStatuses.lockConfusionTurns = 0;

        // If the target can be confused, confuse them.
        // Don't use CanBeConfused, can cause issues in edge cases.
        if (!(GetBattlerAbility(otherSkyDropper) == ABILITY_OWN_TEMPO
            || gBattleMons[otherSkyDropper].volatileStatuses.confusionTurns > 0
            || IsBattlerTerrainAffected(otherSkyDropper, STATUS_FIELD_MISTY_TERRAIN)))
        {
            // Set confused status
            gBattleMons[otherSkyDropper].volatileStatuses.confusionTurns = ((Random()) % 4) + 2;
    ...
    if (!(gBattleMons[otherSkyDropper].volatileStatuses.lockConfusionTurns != 0) && gBattleStruct->skyDropTargets[battler] < 4)
    {
...
{
    const u8 *result = NULL;
    gBattleMons[battler].volatileStatuses.uproarTurns = 0;
    gBattleMons[battler].volatileStatuses.bideTurns = 0;

    if (B_RAMPAGE_CANCELLING < GEN_5)
    {
        gBattleMons[battler].volatileStatuses.multipleTurns = 0;
        gBattleMons[battler].volatileStatuses.lockConfusionTurns = 0;
    }
    else if (gBattleMons[battler].volatileStatuses.lockConfusionTurns == 0
     || gBattleMons[battler].volatileStatuses.lockConfusionTurns > 1)
    {
        gBattleMons[battler].volatileStatuses.multipleTurns = 0;
    }
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
    ...
    if (DYNAMAX_BYPASS_CHECK && GetActiveGimmick(battler) != GIMMICK_Z_MOVE && move == gLastMoves[battler] && move != MOVE_STRUGGLE && (gBattleMons[battler].volatileStatuses.torment == TRUE))
    {
...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Disable
            ...
            unusableMoves |= 1u << i;
        // Torment
        else if (check & MOVE_LIMITATION_TORMENTED && move == gLastMoves[battler] && gBattleMons[battler].volatileStatuses.torment == TRUE)
            unusableMoves |= 1u << i;
        // Taunt
...
void TryClearRageAndFuryCutter(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (gBattleMons[i].volatileStatuses.rage && gChosenMoveByBattler[i] != MOVE_RAGE)
            gBattleMons[i].volatileStatuses.rage = FALSE;
...
static inline bool32 TryActivatePowderStatus(u32 move)
{
    ...
    if (!gBattleMons[gBattlerAttacker].volatileStatuses.powder)
        return FALSE;
...
{
    gBattleMons[gBattlerAttacker].volatileStatuses.destinyBond = FALSE;
...
{
    if (gBattleMons[gBattlerAttacker].volatileStatuses.recharge)
    {
        gBattleMons[gBattlerAttacker].volatileStatuses.recharge = TRUE;
...
{
    if (gBattleMons[gBattlerAttacker].volatileStatuses.flinched)
    {
...
{
    if (!gBattleStruct->isAtkCancelerForCalledMove && gBattleMons[gBattlerAttacker].volatileStatuses.infatuation)
    {
        gBattleScripting.battler = gBattleMons[gBattlerAttacker].volatileStatuses.infatuation - 1;
...
static void CancellerConfused(u32 *effect)
{
    ...
    if (gBattleMons[gBattlerAttacker].volatileStatuses.confusionTurns > 0)
    {
        if (!(gStatuses4[gBattlerAttacker] & STATUS4_INFINITE_CONFUSION))
            gBattleMons[gBattlerAttacker].volatileStatuses.confusionTurns--;
        if (gBattleMons[gBattlerAttacker].volatileStatuses.confusionTurns > 0)
        {
             // confusion dmg
...
{
    if (gBattleMons[gBattlerAttacker].volatileStatuses.bideTurns != 0)
    {
        if (--gBattleMons[gBattlerAttacker].volatileStatuses.bideTurns != 0)
        {
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_SOUNDPROOF:
        ...
        {
            if (gBattleMons[battlerAtk].volatileStatuses.multipleTurns)
                gHitMarker |= HITMARKER_NO_PPDEDUCT;
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_IMPOSTER:
            {
                ...
                if (IsBattlerAlive(diagonalBattler)
                    && !(gBattleMons[diagonalBattler].volatileStatuses.transformed
                        || gBattleMons[diagonalBattler].volatileStatuses.substitute)
        ...
        case ABILITY_COMMANDER:
             ...
             && GET_BASE_SPECIES_ID(GetMonData(GetBattlerMon(battler), MON_DATA_SPECIES)) == SPECIES_TATSUGIRI)
            {
                ...
                if (gBattleMons[battler].volatileStatuses.confusionTurns > 0
                 && !(gStatuses4[battler] & STATUS4_INFINITE_CONFUSION))
                    gBattleMons[battler].volatileStatuses.confusionTurns--;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_CUTE_CHARM:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && !(gBattleMons[gBattlerAttacker].volatileStatuses.infatuation)
             ...
             && !IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
            {
                gBattleMons[gBattlerAttacker].volatileStatuses.infatuation = gBattlerTarget + 1;
...
bool32 CanBattlerEscape(u32 battler) // no ability check
{
    if (gBattleStruct->commanderActive[battler] != SPECIES_NONE)
        ...
        return TRUE;
    else if (gBattleMons[battler].volatileStatuses.escapePrevention)
        return FALSE;
    else if (gBattleMons[battler].volatileStatuses.wrapped)
        return FALSE;
...
{
    if (gDisableStructs[battler].boosterEnergyActivates || gBattleMons[battler].volatileStatuses.transformed)
        return ITEM_NO_EFFECT;
...
static u8 ItemEffectMoveEnd(u32 battler, enum ItemHoldEffect holdEffect)
{
    ...
    case HOLD_EFFECT_CURE_CONFUSION:
        if (gBattleMons[battler].volatileStatuses.confusionTurns > 0 && !UnnerveOn(battler, gLastUsedItem))
        {
            RemoveConfusionStatus(battler);
    ...
    case HOLD_EFFECT_CURE_STATUS:
        if ((gBattleMons[battler].status1 & STATUS1_ANY || gBattleMons[battler].volatileStatuses.confusionTurns > 0) && !UnnerveOn(battler, gLastUsedItem))
        {
...
u8 GetAttackerObedienceForAction()
{
    ...
    if (moveEffect == EFFECT_RAGE)
        gBattleMons[gBattlerAttacker].volatileStatuses.rage = FALSE;
    ...
    else
    {
        ...
        if (calc < obedienceLevel && CanBeSlept(gBattlerAttacker, gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), NOT_BLOCKED_BY_SLEEP_CLAUSE))
        {
            ...
            int i;
            for (i = 0; i < gBattlersCount; i++)
                if (gBattleMons[i].volatileStatuses.uproarTurns)
                    break;
            if (i == gBattlersCount)
...
u32 CalcRolloutBasePower(u32 battlerAtk, u32 basePower, u32 rolloutTimer)
{
    ...
    for (i = 1; i < (5 - rolloutTimer); i++)
        basePower *= 2;
    if (gBattleMons[battlerAtk].volatileStatuses.defenseCurl)
        basePower *= 2;
    return basePower;
}
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            ...
            if (((weather & B_WEATHER_SUN && HasWeatherEffect()) || gDisableStructs[battlerDef].boosterEnergyActivates)
             ...
             && !(gBattleMons[battlerDef].volatileStatuses.transformed))
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_ROCKY_PAYLOAD:
        ...
        break;
    case ABILITY_PROTOSYNTHESIS:
        if (!(gBattleMons[battlerAtk].volatileStatuses.transformed))
        {
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectDef, u32 weather)
{
    ...
    case HOLD_EFFECT_METAL_POWDER:
        if (gBattleMons[battlerDef].species == SPECIES_DITTO && usesDefStat && !(gBattleMons[battlerDef].volatileStatuses.transformed))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST && gBattleMons[battlerDef].volatileStatuses.foresight && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
...
#define UNPACK_V_STATUS_SETTERS(_enum, _type, _fieldNameBitSize, ...) case _enum: INVOKE(UNPACK_V_STATUS_SETTERS_, _type, UNPACK(_fieldNameBitSize)); break;
